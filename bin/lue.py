...
STREAMTYPES = {
    ...
    0x6 :("ISO 13818-1 PES private data", "p"),
    0x7: ("ISO 13522 MHEG", "-"),
...
def parsePMT():
    ...
    program_info_length = int.from_bytes(
        spacket[start+11:start+13], byteorder='big') & 0b0000111111111111
    program_info = spacket[start+13:start+13+program_info_length]

    section_length = int.from_bytes(
        spacket[start+2:start+4], byteorder='big') & 0b0000111111111111
    program_number = int.from_bytes(
        spacket[start+4:start+6], byteorder='big')
    version_number = (spacket[start+6] & 0b00111110) >> 1
    current_next_indicator = (spacket[start+6] & 0b00000001)
    ...
    while pointer < section_length+program_info_length-17:
        ...
        extdata = spacket[pointer+5:pointer+ES_info_length+5]
        print(extdata)
        for e in extdata: 
            print(hex(e),end=",")
        print()
        if STREAMTYPES[streamType][1]=="a":
            otype="audio"
        ...
        #         # and spacket[i+1]==4:
        #         if spacket[i] == 89 or spacket[i] == 86:
        #             # try:
        #             lng = spacket[i+2:i+5].decode()
        #             subtype = spacket[i+5]
        ...
        #                 ssubtype = "teletext"  # teletext
                    ...
                    # except:
                    #     pass
        print("TYPE", hex(streamType), hex(elementary_PID), olng, otype)
        # if streamType == 6:
...
if __name__ == "__main__":
    spacno = 0
    with open("/home/pulla/Desktop/ts/sf.ts", "rb") as f:
        while True:
            spacket = f.read(188)
            if not spacket:
                break
